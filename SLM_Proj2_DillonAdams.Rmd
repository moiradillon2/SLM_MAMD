...
# New data set as predictor -- used sqrt(1stQ), sqrt(Median), sqrt(3rdQ) -- not sure if this should be with sqrt or not
...
#bonn <- 
#ggplot(lahman_pred, aes(x = sqrt.salary., y = sqrt.G.)) + 
#    geom_point(size=2.5) + 
#    geom_line(aes(y=fitted, x=sqrt.salary.), size=1) + 
...
ggplot(lahman_pred, aes(x=sqrt.salary., y = sqrt.G.)) + geom_point(size=1, shape=1) +
  geom_line(aes(y=fitted, x=sqrt.salary.), size = 1) + 
...
```{r, eval=TRUE, warning=FALSE}
#Using the values for bonf_crit and WH_crit we calculated in the previous chunk
#Prediction intervals for WH
#WH.upper.pred <- fitted + crit_Schef * se.pred
#WH.lower.pred <- fitted - crit_Schef * se.pred
#Prediction intervals for Bonf
#Bonf.lower.pred <- fitted + crit_Bonf * se.pred
#Bonf.upper.pred <- fitted - crit_Bonf * se.pred
```
